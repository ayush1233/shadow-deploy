...
const nodes: ServiceNode[] = [
    // Row 1: Client → Proxy → Prod/Shadow
    ...
    // Row 2: Ingestion → Kafka (wide spread)
    ...
    // Row 3: Comparison → AI (much wider gap)
    ...
    // Row 4: Dashboard + API (separate row, wide gaps)
...
// Add Redis, Prometheus, Grafana if detected in docker-compose
    ...
    return (
        <div style={{ padding: 32, animation: 'fadeIn 0.5s ease' }}>
            ...
            <div style={{ marginBottom: 20 }}>
                <h2 style={{ fontSize: 20, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                    How This Project Works
                </h2>
                <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginTop: 4 }}>
                    Architecture auto-detected from <code style={{ fontSize: 12, padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.06)' }}>nginx.conf</code> and <code style={{ fontSize: 12, padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.06)' }}>docker-compose.yml</code>
                </p>
            </div>

            {/* Source info banner */}
            <div style={{
                display: 'flex', gap: 16, marginBottom: 16, padding: '12px 16px', borderRadius: 10,
                ...
                <span style={{ fontSize: 12, color: '#10b981' }}>
                    🟢 Production on <strong>:{topoConfig.prodPort}</strong>
                </span>
                <span style={{ fontSize: 11, color: 'var(--text-secondary)', opacity: 0.6 }}>|</span>
                <span style={{ fontSize: 12, color: '#f59e0b' }}>
                    👻 Shadow on <strong>:{topoConfig.shadowPort}</strong>
                </span>
                <span style={{ fontSize: 11, color: 'var(--text-secondary)', opacity: 0.6 }}>|</span>
                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                    🔀 Proxy on <strong>:{topoConfig.proxyPort}</strong>
                </span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 24, marginBottom: 16, flexWrap: 'wrap' }}>
                {[
                    ...
                    <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 24, height: 3, borderRadius: 2, background: l.color, ...(l.dash ? { backgroundImage: `repeating-linear-gradient(90deg, ${l.color} 0 6px, transparent 6px 10px)`, background: 'transparent' } : {}) }} />
                        ...
                        return (
                            <g key={`c${i}`} onMouseEnter={() => setHoveredConn(i)} onMouseLeave={() => setHoveredConn(null)}
                                ...
                                {isH && (() => {
                                    ...
                                    // offset label away from nodes to avoid overlap
                                ...
                                {node.port > 0 && (
                                    <>
                                        ...
                                        <text x={node.x + 52} y={node.y + 52} fontSize={11}
                                            fill="var(--text-secondary)" fontFamily="var(--font-family)">:{node.port}</text>
                                    </>
                                )}
                                ...
                                {isH && (
                                    <g>
                                        ...
                                        <text x={node.x + 80} y={node.y + 90} textAnchor="middle" fontSize={10}
                                            fill="var(--text-secondary)" fontFamily="var(--font-family)">{node.desc}</text>
                                    </g>
                                )}
                            </g>
                        );
            ...
            {/* Service Grid — all detected services with ports */}
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(185px, 1fr))', gap: 10 }}>
                {allNodes.filter(n => n.port > 0).map(node => {
                    const s = nodeStyles[node.type] || fallback;
                    const isH = hoveredNode === node.id;
                    return (
                        <div key={node.id} className="card" style={{
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 1 }}>localhost:{node.port}</div>
                            </div>
                            <div style={{
            ...
            {/* Source footer */}
            <div style={{ marginTop: 16, fontSize: 11, color: 'var(--text-secondary)', opacity: 0.5, textAlign: 'center' }}>
                Auto-generated from project config • {topoConfig.services.length} services detected • Last extracted: {new Date(topoConfig._generatedAt).toLocaleString()}
            </div>
        </div>
    );
}
